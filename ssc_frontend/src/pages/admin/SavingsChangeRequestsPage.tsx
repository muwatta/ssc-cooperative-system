...
import { PageHeader, EmptyState, formatNaira } from "@/components/common";
      ...
      <div className="card w-full max-w-md dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="card-body space-y-4">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Approving{" "}
            <strong className="dark:text-white">{request.member_name}</strong> (
          ...
          <div>
            <label className="label">Effective Hijri Month</label>
            <select
              value={month}
              onChange={(e) => setMonth(Number(e.target.value))}
              className="input"
              id="hijri-month-select"
              title="Select the Hijri month when the change should take effect. This determines when the new contribution amount will be applied to the member's account."
            >
              {HIJRI_MONTHS.map((m) => (
          ...
          <div>
            <label className="label">Effective Hijri Year</label>
            <input
              type="number"
              value={year}
              ...
              title="Enter the Hijri year when the change should take effect. This determines when the new contribution amount will be applied to the member's account."
            />
          </div>
          ...
          {["pending", "approved", "rejected"].map((status) => (
            ...
            >
              ...
              {status === "pending" && requests.length > 0 && (
                <span className="ml-2 rounded-full bg-primary-100 px-2 py-0.5 text-xs text-primary-700 dark:bg-primary-900/50 dark:text-primary-300">
                  {requests.length}
                </span>
              )}
            </button>
      ...
      {requests.length === 0 ? (
        ...
        <div className="space-y-4">
          {requests.map((req) => {
            ...
            return (
              <div key={req.id} className="card-panel p-5">
