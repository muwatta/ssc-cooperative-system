...
def admin_final_approve_loan(loan: LoanApplication, admin_user, note: str = "") -> LoanApplication:
    ...
    # Calculate self-surety amount to lock (e.g. 75% of approved amount)
    self_surety_amount = (loan.amount_approved * config.self_surety_ratio).quantize(Decimal("0.01"))

    # Lock the self-surety portion into suretyship_committed —
    # do NOT debit the full loan from savings. The loan is disbursed
    # from the cooperative fund, not the member's personal savings.
    ...
    # Record the self-surety lock in the savings ledger for audit trail
    post_debit_entry(
        member=loan.applicant,
